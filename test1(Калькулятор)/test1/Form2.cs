...
namespace test1
{
    public partial class Form2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)//Очистка текстового поля ввода
        {
            textBox1.Clear();
            textBox1.Focus();
        }
        private void textBox1_TextChanged(object sender, EventArgs e)// Воспроизведение результата
        {
            string text = textBox1.Text;
            if(!textBox1.Text.Equals("") && textBox1.Text.EndsWith("="))//Если нажата клавиша "="
            {
                text = text.Replace(" ", "");//убираем пробелы
                text = text.Replace("=", "");// и вводится "="
                textBox1.Text += textBox1RPF(DoRPF(text));// высчитываем введенные данные и выводим результат
            }
        }
        ...
        public static double textBox1RPF(string input) //Вычисление выражения в ОПФ
        {
            double result = 0;
